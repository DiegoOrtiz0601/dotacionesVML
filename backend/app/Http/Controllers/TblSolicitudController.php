...
class TblSolicitudController extends Controller
{
    ...
    public function indexGestionar(Request $request)
    {
        ...
        // Aplicar filtro por empresa (opcional) - usa índice idx_solicitudes_empresa_sede
        if ($request->filled('empresa')) {
            $query->where('s.idEmpresa', $request->empresa);
        }

        // Aplicar filtro por sede (opcional) - usa índice idx_solicitudes_empresa_sede
        ...
        if ($request->filled('estado')) {
            $query->where('s.estadoSolicitud', $request->estado);
        }

        // Optimización: Usar índices existentes para ordenamiento
        // El índice idx_solicitudes_created_at mejora el ORDER BY
        $perPage = $request->get('per_page', 10);
        
        // Paginación de resultados con optimización
    ...
    public function aprobar(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->input('generarExcel', false)) {
                try {
                    Log::info('🔄 Iniciando generación de Excel para solicitud: ' . $id);
                    
                    $excelController = new ExcelSolicitudController();
                    $excelResult = $excelController->generarExcelSolicitudAprobada($id, $request);
                    
                    Log::info('📊 Resultado de generación Excel:', (array) $excelResult->getData());
                    
                    if ($excelResult->getData()->success) {
                        $excelGenerado = true;
                        $excelInfo = [
                            'filename' => $excelResult->getData()->filename,
                            'download_url' => $excelResult->getData()->download_url,
                            'file_path' => $excelResult->getData()->file_path
                        ];
                        
                        Log::info('✅ Excel generado exitosamente:', $excelInfo);
                    } else {
                        Log::error('❌ Error en respuesta de Excel:', $excelResult->getData());
                    }
                } catch (\Exception $e) {
                ...
                Log::info('ℹ️ No se solicitó generar Excel');
            }

            // Enviar correos con información del Excel si se generó
            Mail::to($correoCompras)->send(new NotificacionSolicitud($solicitud, $empleadosMail, 'compras', $excelInfo));
            if ($usuarioCreador && $usuarioCreador->CorreoUsuario) {
                Mail::to($usuarioCreador->CorreoUsuario)->send(new NotificacionSolicitud($solicitud, $empleadosMail, 'usuario', $excelInfo));
            }
            ...
            if ($excelGenerado) {
                $response['excel_generado'] = true;
                $response['excel_info'] = $excelInfo;
            }
    ...
    public function historialSolicitudes(Request $request)
    {
        try {
            $query = DB::table('tbl_solicitudes as s')
                ...
                ->whereIn('s.estadoSolicitud', ['Aprobado', 'Aprobado Parcial', 'Rechazado'])
            ...
            if ($request->filled('fecha_desde')) {
                $query->where('s.updated_at', '>=', $request->fecha_desde);
            }

            if ($request->filled('fecha_hasta')) {
                $query->where('s.updated_at', '<=', $request->fecha_hasta . ' 23:59:59');
            }
            ...
            $estadisticas = [
                'total' => $result->total(),
                'aprobadas' => $result->where('estado', 'Aprobado')->count(),
                'aprobadas_parcial' => $result->where('estado', 'Aprobado Parcial')->count(),
                'rechazadas' => $result->where('estado', 'Rechazado')->count(),
            ];
            ...
            Log::error('❌ Error obteniendo historial de solicitudes: ' . $e->getMessage());
            return response()->json(['message' => 'Error interno del servidor'], 500);
        }
    }
