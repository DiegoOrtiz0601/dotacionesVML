...
// Componente principal para mostrar el resumen de una solicitud de dotación
function ResumenSolicitudVista({
  ...
  // Obtener datos amigables de empresa y sede
  const nombreEmpresa =
      ...
      <div className="bg-white rounded shadow-md border border-gray-300 p-6 mt-6">
        ...
        <div className="flex justify-between items-center border-b pb-4 mb-4">
          ...
          <div className="text-center">
            <h3 className="text-xl font-bold text-primario">🧾 Solicitud de Dotación</h3>
          </div>
        ...
        <div>
          ...
          {resumenSolicitud.map((emp) => (
            <div
              key={emp.documentoEmpleado || emp.idDetalleSolicitud || crypto.randomUUID()}
              className="border-t pt-4 mt-4"
            >
              <div>
                <p className="font-semibold text-base">
                  {emp.nombreEmpleado || emp.nombresEmpleado} (Documento: {emp.documentoEmpleado})
                </p>
                <p className="text-sm text-gray-600 mb-2">
                  Cargo: {emp.cargo} | Tipo solicitud: {emp.tipoSolicitud || emp.NombreTipo || "N/A"}
                </p>
              </div>
              ...
              <details className="mt-2 bg-gray-50 border rounded p-3">
                ...
                <ul className="list-disc ml-5 mt-2 text-sm text-gray-700">
                  {(emp.elementos || []).map((el) => (
                    <li key={`${el.idElemento || el.nombreElemento}-${el.talla}`}>
                      {el.nombreElemento || "Elemento"} — Talla: <strong>{el.talla || el.TallaElemento}</strong>, Cantidad: <strong>{el.cantidad || el.Cantidad}</strong>
                    </li>
                  ))}
                </ul>
